...
function deathResults(){
  ...
  if(charStatus.indexOf('bird death') > -1 ){
    deathBy = readSummaryAndAddToTotal('bird death');
  ...
  if(charStatus.indexOf('Dalmatian death') > -1 ){
    deathBy = readSummaryAndAddToTotal('Dalmatian death');
  ...
  if(charStatus.indexOf('Retriever death') > -1 ){
    deathBy = readSummaryAndAddToTotal('Retriever death');
  ...
  if(deathTxt.length === 0) {
    deathBy = readSummaryAndAddToTotal('Did not die');
    ...
    deathBy = readSummaryAndAddToTotal('Did die');
  }
  console.log(total);
  console.log(deathBy);
...
function injuryResults(){
  ...
  if(charStatus.indexOf('bird_injury') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Wind up window');
  }
  if(charStatus.indexOf('shop_injury') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Drove through windows');
  }
  if(charStatus.indexOf('bird_injury2') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Fought rats');
  }
  if(charStatus.indexOf('bite wine') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Dog fight: Defended with wine bottle');
  }
  if(charStatus.indexOf('bite wine broken') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Dog fight: Attacked with broken bottle');
  }
  if(charStatus.indexOf('bite wine broken') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Dog fight: Defended with the hammer');
  }
  if(charStatus.indexOf('bite mop') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Dog fight: Attacked with a mop');
  }
  if(charStatus.indexOf('bite hand') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Dog fight: unarmed');
  }
  if(charStatus.indexOf('bite bread') > -1 ){
    ...
    injured = readSummaryAndAddToTotal('Dog fight: Bread');
  }
  if(injuryTxt.length === 0){
    ...
    if(didntDie){
      ...
      injured = readSummaryAndAddToTotal('Uninjured and did not die');
    } else {
      injuryTxt += "<br>If it weren't for all that death, I would have called you a true survivor of the  animal apocalypse.";
      injured = readSummaryAndAddToTotal('Uninjured but died');
    }
  } else {
    injured = readSummaryAndAddToTotal('Injured');
  }
  console.log(total);
  console.log(injured);
...
function checkCompanion(){
  ...
  if(charStatus.indexOf('companion left') > -1 ){
    injured = readSummaryAndAddToTotal('Rescued man but then chose dog to survive.');
  ...
  if(charStatus.indexOf('companion remain') > -1 ){
    injured = readSummaryAndAddToTotal('Made it out with friend');
  ...
  if(charStatus.indexOf('cpr failed') > -1 ){
    injured = readSummaryAndAddToTotal('Failed at life-saving');
    ...
    injured = readSummaryAndAddToTotal('Never helped stranger');
  ...
  if(name == 'John' && charStatus.indexOf('no_friend') === -1){
    injured = readSummaryAndAddToTotal('Saved John');
    genderStatus = 'Did you know, you can also save a woman (Mary) instead of John?'    
  } else {
    injured = readSummaryAndAddToTotal('Saved Mary');
  }
